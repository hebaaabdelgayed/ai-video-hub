...
<body>
  ...
  <footer class="site-footer">
    <div class="footer-inner"><p>© ${new Date().getFullYear()} ${escapeHtml(config.arabicName)}. مكتبة منظمة لفيديوهات القناة وزيادة قابلية الوصول إليها.</p></div>
  </footer>
</body>
</html>`;
}
...
function videoCard(video) {
  ...
  return `<article class="video-card">
    <a href="${route(path)}">${video.thumbnail ? `<img src="${escapeHtml(video.thumbnail)}" alt="${escapeHtml(video.title)}" loading="lazy">` : ''}</a>
    <div class="card-body">
...
function sectionHeader(title, text, actionHref, actionText) {
  return `<div class="section-heading"><div><h2>${escapeHtml(title)}</h2>${text ? `<p class="lead">${escapeHtml(text)}</p>` : ''}</div>${actionHref ? `<a class="button secondary" href="${actionHref}">${escapeHtml(actionText)}</a>` : ''}</div>`;
}

function compactVideoList(videos) {
  return `<ul class="compact-list">${videos.map((video) => `<li><a href="${route(`/videos/${video.seo.slug}/`)}">${escapeHtml(video.title)}</a><span>${formatDate(video.publishedAt)}</span></li>`).join('')}</ul>`;
}

function videosForTopic(slug, count = 3) {
  return data.videos.filter((video) => video.seo?.topics?.some((topic) => topic.slug === slug)).slice(0, count);
}
...
function learningPathCard(topicSlug, title, text) {
  ...
  return `<article class="path-card"><div class="path-card-main"><h3><a href="${href}">${escapeHtml(title)}</a></h3><p>${escapeHtml(text)}</p><p class="meta">${topic ? `${topic.count.toLocaleString('ar')} فيديو` : 'مسار تعليمي'}</p></div>${compactVideoList(videos)}</article>`;
}

function latestVideos(count = 6) {
  return data.videos.slice(0, count).map(videoCard).join('');
}

writePage('', layout({
  title: `${config.arabicName} | مكتبة فيديوهات الذكاء الاصطناعي`,
  ...
  body: `<main>
    <section class="hero">
      <div>
        ...
        <div class="hero-actions"><a class="button" href="${route('/topics/')}">ابدأ من المواضيع</a><a class="button secondary" href="${route('/videos/')}">كل الفيديوهات</a><a class="button secondary" href="${escapeHtml(config.channelUrl)}">قناة يوتيوب</a></div>
      </div>
      <div class="stats" aria-label="إحصاءات القناة"><div class="stat"><strong>${data.videos.length.toLocaleString('ar')}</strong><span>فيديو منظم</span></div><div class="stat"><strong>${data.playlists.length.toLocaleString('ar')}</strong><span>قائمة تشغيل</span></div><div class="stat"><strong>${data.topics.length.toLocaleString('ar')}</strong><span>موضوع AI</span></div></div>
    </section>
    ...
    <section class="section">${sectionHeader('أحدث الفيديوهات', 'آخر الشروحات والتجارب المنشورة على القناة.', route('/videos/'), 'عرض كل الفيديوهات')}<div class="grid">${latestVideos(6)}</div></section>
    <section class="section">${sectionHeader('المواضيع الرئيسية', 'تصنيفات واسعة تساعدك تصل للفيديو المناسب حسب الأداة أو مجال الاستخدام.', route('/topics/'), 'كل التصنيفات')}<div class="grid">${data.topics.slice(0, 12).map((topic) => `<a class="topic-card card-body" href="${route(`/topics/${topic.slug}/`)}"><h3>${escapeHtml(topic.name)}</h3><p class="meta">${topic.count.toLocaleString('ar')} فيديو</p></a>`).join('')}</div></section>
    <section class="section trust-section"><div><h2>لماذا هذا الموقع؟</h2><p class="lead">القناة فيها أكثر من ٢٠٠ فيديو. هذا الموقع يحولها إلى مكتبة منظمة قابلة للبحث، مع صفحات لكل فيديو وروابط بين المواضيع، حتى يكون الوصول للشروحات أسهل من البحث العشوائي.</p></div><div class="trust-points"><div><strong>منظم حسب الهدف</strong><span>أدوات، نماذج، دراسة، تصميم، برمجة، وأتمتة.</span></div><div><strong>مرتبط بيوتيوب</strong><span>كل صفحة تقودك للفيديو الأصلي أو تشغله مباشرة.</span></div><div><strong>قابل للتوسع</strong><span>كل فيديو جديد يمكن إضافته تلقائيا بعد النشر.</span></div></div></section>
  </main>`
}));
...
for (const video of data.videos) {
  const related = data.videos.filter((candidate) => candidate.id !== video.id && candidate.seo?.topics?.some((topic) => video.seo?.topics?.some((own) => own.slug === topic.slug))).slice(0, 4);
  const path = `/videos/${video.seo.slug}/`;
  writePage(`videos/${video.seo.slug}`, layout({
    title: video.seo.title,
    description: video.seo.metaDescription,
    path,
    jsonLd: { '@context': 'https://schema.org', '@type': 'VideoObject', name: video.title, description: video.seo.metaDescription, thumbnailUrl: video.thumbnail ? [video.thumbnail] : [], uploadDate: video.publishedAt, duration: video.duration, embedUrl: `https://www.youtube.com/embed/${video.id}`, url: video.youtubeUrl },
    body: `<main class="video-layout"><article><iframe class="video-frame" src="https://www.youtube.com/embed/${video.id}" title="${escapeHtml(video.title)}" allowfullscreen></iframe><div class="panel"><h1 class="page-title">${escapeHtml(video.title)}</h1><p class="meta">${formatDate(video.publishedAt)} · ${video.viewCount?.toLocaleString('ar') || 0} مشاهدة</p><p>${escapeHtml(video.seo.summary)}</p><div class="tag-list">${video.seo.keywords.map((keyword) => `<span class="tag">${escapeHtml(keyword)}</span>`).join('')}</div><div class="actions"><a class="button" href="${escapeHtml(video.youtubeUrl)}">فتح الفيديو على يوتيوب</a></div></div><section class="panel"><h2>أسئلة سريعة</h2>${video.seo.faq.map((item) => `<h3>${escapeHtml(item.question)}</h3><p>${escapeHtml(item.answer)}</p>`).join('')}</section></article><aside><div class="panel"><h2>المواضيع</h2><div class="tag-list">${video.seo.topics.map((topic) => `<a class="tag" href="${route(`/topics/${topic.slug}/`)}">${escapeHtml(topic.name)}</a>`).join('')}</div></div><div class="panel"><h2>فيديوهات ذات صلة</h2><ul class="list">${related.map((item) => `<li><a href="${route(`/videos/${item.seo.slug}/`)}">${escapeHtml(item.title)}</a></li>`).join('')}</ul></div></aside></main>`
  }));
}
